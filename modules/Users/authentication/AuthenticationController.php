...
class AuthenticationController implements LoggerAwareInterface
{
    ...
    $this->logger->debug(sprintf('TCT: username:'.$username));
    $this->logger->debug(sprintf('TCT: password_'. $password));
    $this->logger->debug(print_r($params, true));
    
    $this->logger->debug(sprintf('TCT: loggedInB'. $this->loginSuccess));
		if($this->loggedIn)return $this->loginSuccess;
    $this->logger->debug(sprintf('TCT: loggedInA'. $this->loginSuccess));
		if(empty($params['noHooks'])) {
        $this->logger->debug(sprintf('TCT: call_custom_logicB'. $this->loginSuccess));
		    LogicHook::initialize()->call_custom_logic('Users', 'before_login');
        $this->logger->debug(sprintf('TCT: call_custom_logicA'. $this->loginSuccess));
		}
        $this->logger->debug(sprintf('TCT: externalLoginB'. $_SESSION['externalLogin']));
        $_SESSION['externalLogin'] = false;
        $this->loggedIn = false;
        $this->loginSuccess = false;
        $this->logger->debug(sprintf('TCT: externalLoginA'. $_SESSION['externalLogin']));
        try {
            $this->loginSuccess = $this->authController->loginAuthenticate($username, $password, false, $params);
            $this->logger->debug(sprintf('loginSuccess'. $this->loginSuccess));
